...
aws s3 cp public s3://"${SERVICE_NAME}"-"${TARGET_ACCOUNT_ID}"-"${PARTNER}"-"${STAGE}" --recursive
...
if [ "$STATUS" = "CREATE_COMPLETE" ]; then
  ...
  echo "The cloudformation stack set is currently being updated. Waiting for 30 seconds..."
  sleep 30
  STATUS=$(aws cloudformation describe-stacks --region "us-east-1" --stack-name "${SERVICE_NAME}"-"${PARTNER}"-"${STAGE}" --output text --query "Stacks[0].StackStatus")
  if [ "$STATUS" = "UPDATE_IN_PROGRESS" ]; then
    echo "Update still in progress after waiting. Exiting..."
    exit 1
  elif [ "$STATUS" = "UPDATE_COMPLETE" ]; then
